...
- Tradeoffs: 현재는 fixture 기반 카드 피드이며 API 연결 전까지 하나의 승인 정책으로 동작한다.
...
- New/changed components: 정책 카드, 상태 배지, 로컬 전용 안내, 카드 피드 빈 상태, 하단 탐색
...
- Offline/slow network, if applicable: 정책 fixture를 사용하므로 기본 데모 화면은 네트워크 없이도 보인다.
