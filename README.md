...
- **동적 로컬 프로필**: 새 조건을 `FieldDefinitionProposal`로 제안하고 승인된 질문만 시민 기기에 추가합니다.
...
| 영역 | 현재 상태 |
|---|---|
| Agent Backend | 공식 공고 수집, 다중 형식 분석, OCR, 이전 공고 diff, PolicyPackage·AgentRun, 검토·공개 API 구현 |
| 시민 PWA | IndexedDB 프로필, 결정론적 matcher, 추가 질문과 승인 정책 API adapter 구현. CloudFront 배포 완료 |
| 관리자 웹 | 검토 UI, 실행 로그와 수동 새 공고 확인 구현. CloudFront 배포 완료 |
| AWS | CloudFront, 비공개 S3, ALB, private ECS Fargate, RDS PostgreSQL, ECR, Secrets Manager와 NAT Gateway로 구성. 프론트엔드와 백엔드 배포 확인 |

백엔드는 83개 테스트를 통과했습니다. 시민 PWA는 8개 matcher 테스트와 production build, 관리자 웹은 production build를 통과한 상태를 기준으로 작성했습니다.
...
flowchart LR
    ...
    AGENT --> STORE["정책 후보 · AgentRun<br/>SQLite / PostgreSQL"]
...
flowchart TB
    ...
    ATTACHMENTS --> REVIEW_FILES["review-attachments/<br/>관리자 presigned URL 연결 예정"]
...
인프라는 Terraform으로 관리하는 구성이며 Git push 자동 배포는 사용하지 않습니다. 프론트엔드는 build 후 S3 업로드와 CloudFront 무효화, 백엔드는 Docker build 후 ECR push와 ECS rolling deployment가 필요합니다. 현재 백엔드는 `public-attachments/` 공개 archive를 구현했지만 `review-attachments/` 업로드와 presigned URL 발급은 애플리케이션 연결 전입니다.
...
> 현재 새 공고 확인은 관리자 수동 실행 방식입니다. 주기 스케줄러와 실시간 푸시는 포함하지 않으며, 배포 환경 전체 흐름은 통합 검증 전입니다.
...
| Amazon S3 | 승인된 공개 근거를 `public-attachments/`에 보관하고 CloudFront 고정 URL로 제공하기 위해 사용했습니다. `review-attachments/`의 presigned URL 연결은 아직 남아 있습니다. |
...
| 팀원 | 주요 기여 | Agent 협업 |
|---|---|---|
| **팀장 · [dlfjsld1](https://github.com/dlfjsld1)** | 프로젝트 기획, 시스템·백엔드 아키텍처, 강남구 공식 공고 수집, 문서 분석·OCR 전략, 이전 공고 비교, 근거 검증, 정책 API와 LangGraph 전체 구조·통합 | 각 처리 노드의 상태와 분기, 실행 로그, 실패·검토 경로를 설계하고 Field Registry·Human Review 결과를 전체 Graph에 연결했습니다. |
| **[이지혜 · C-JJI](https://github.com/C-JJI)** | 시민 PWA, 개인정보 중심 온보딩, 동적 로컬 프로필과 IndexedDB, 재귀 EligibilityRule matcher, `YES / NO / UNKNOWN / STALE` 판정·추가 질문, 정책 피드와 사용자 경험 | **Field Registry Agent 로직**을 함께 구현해 기존 canonical field 재사용, 신규 `FieldDefinitionProposal` 생성, 모호한 후보와 중복 제안 방지 흐름을 완성했습니다. |
| **[김준 · kimjun-dev](https://github.com/kimjun-dev)** | 관리자 검토·승인 화면, Agent 실행 로그와 근거 표시, 프론트·백엔드 통합, AWS 인프라·배포와 운영 환경 연결 | **Human-in-the-loop Agent 로직**을 함께 구현해 `review_required`, `review_reason`, `unresolved_fields`, `AgentRun`과 Publish 전 검토 객체 생성 흐름을 완성했습니다. |
