...
- Current milestone: 관리자 검토 API와 Publish
- Working: health API, 승인 정책 fixture API, 실제 Agent 실행·조회 API, 공통 계약, 공식 게시판 수집, HTML·PDF·HWPX 추출, 최후 복구용 OpenAI 이미지 OCR adapter, 동일 문서 변형 교차 검증, 구조화 정책 후보와 PolicyPackage 조립, Field Registry 해석, Human Review 결과 생성, LangGraph 실행 흐름과 AgentRun 로그 누적, SQLite/PostgreSQL 공용 저장소 계층
- In progress: 관리자 검토 API와 승인된 PolicyPackage Publish
- Not implemented: 관리자 승인·수정·반려 API, 승인 후 Publish, PostgreSQL live integration
- Blockers: none
...
- [ ] FieldDefinitionReview 승인·수정·반려 API를 저장소와 연결한다.
- [ ] 승인된 PolicyPackage만 시민 API에 공개하도록 Publish 흐름을 연결한다.
