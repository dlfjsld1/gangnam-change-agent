...
- Current milestone: 관리자 브라우저 앱 보일러플레이트
- Working: Vite 앱, 환경변수 구조, FieldDefinitionReview·AgentRun 계약
- In progress: 동적 필드 제안과 evidence 검토 화면 설계
- Not implemented: 승인/수정/반려, Agent 로그, 시민 앱 통합, 배포
- Blockers: 계약에 맞춘 관리자 API 구현 필요

## Next actions

- [ ] fixture 기반 FieldDefinitionProposal 목록과 상세 검토 화면을 만든다.
- [ ] 원문 evidence, 제안 필드, 기존 canonical field 후보를 함께 표시한다.
- [ ] 승인·수정·반려 동작을 로컬 fixture 상태로 먼저 연결한다.
- [ ] AgentRun의 review_required, review_reason, unresolved_fields와 실행 로그를 표시한다.
